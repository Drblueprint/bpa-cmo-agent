...
def fb_insights(token: str, account: str, date_preset: str = "last_7d") -> list:
    ...
    if status >= 400:
        print(f"  FB API error {status}: {body.get('error', body)}")
        return []
...
def hyros_paginate(path: str, key: str, params: dict, max_pages: int = 20) -> list:
    ...
    while page <= max_pages:
        ...
        if status >= 400:
            print(f"  Hyros API error {status} on {path}: {body}")
            break
...
def extract_lead_source(lead: dict) -> dict:
    """Pull full attribution from a Hyros lead record.

    Checks firstSource before lastSource to get the originating ad.
    Also reads tags for journey milestones (!hubspot, $conversion events).
    """
    tags = lead.get("tags") or []
    has_hubspot = "!hubspot" in tags
    call_tags = [t for t in tags if t.startswith("$")]

    for src_key in ("firstSource", "lastSource"):
        src = lead.get(src_key) or {}
        if not src:
            continue
        ad_name = src.get("name") or ""
        campaign = (src.get("category") or {}).get("name") or ""
        url = src.get("url") or lead.get("optInUrl") or ""
        creative = (src.get("sourceLinkAd") or {}).get("name") or ""
        ad_set_id = (src.get("adSource") or {}).get("adSourceId") or ""
        traffic_source = (src.get("trafficSource") or {}).get("name") or ""
        if ad_name or campaign:
            return {
                "ad_name": ad_name,
                "campaign": campaign,
                "creative": creative,
                ...
                "call_booked": bool(call_tags),
                "conversion_tags": call_tags,
            }
    return {
        "ad_name": "",
        "campaign": "",
...
def hyros_lead_source_rollup(leads: list) -> dict:
    """Aggregate Hyros leads by originating FB ad, campaign, and creative.

    Also extracts the subset of leads confirmed as HubSpot contacts via
    the !hubspot tag, keyed by email so they can be cross-referenced.
    """
    from collections import defaultdict
    by_ad: dict = defaultdict(int)
    by_campaign: dict = defaultdict(int)
    by_creative: dict = defaultdict(int)
    by_segment: dict = defaultdict(int)
    unattributed = 0
    hubspot_confirmed = 0
    calls_booked = 0
    # email -> attribution dict for leads confirmed in HubSpot
    hubspot_leads_by_email: dict = {}

    call_booked_leads: list = []
    for lead in leads:
        src = extract_lead_source(lead)
        if src["has_hubspot"]:
            hubspot_confirmed += 1
            if src["email"]:
                hubspot_leads_by_email[src["email"]] = src
        if src["call_booked"]:
            calls_booked += 1
            if src["email"]:
                call_booked_leads.append(src)
        if src["ad_name"] or src["campaign"]:
            if src["ad_name"]:
                by_ad[src["ad_name"]] += 1
            if src["campaign"]:
                by_campaign[src["campaign"]] += 1
                by_segment[segment_of(src["campaign"])] += 1
            elif src["ad_name"]:
                by_segment[segment_of(src["ad_name"])] += 1
            if src["creative"]:
                by_creative[src["creative"]] += 1
        else:
            unattributed += 1

    attributed = len(leads) - unattributed
    return {
        "total": len(leads),
        "attributed": attributed,
        "unattributed": unattributed,
        "attribution_rate": (attributed / len(leads) * 100) if leads else 0,
        "hubspot_confirmed": hubspot_confirmed,
        "calls_booked": calls_booked,
        "call_booked_leads": call_booked_leads,
        "hubspot_leads_by_email": hubspot_leads_by_email,
        "by_ad": dict(sorted(by_ad.items(), key=lambda x: -x[1])),
        "by_campaign": dict(sorted(by_campaign.items(), key=lambda x: -x[1])),
        "by_creative": dict(sorted(by_creative.items(), key=lambda x: -x[1])),
...
def render_text(r: dict, window_days: int = 7) -> str:
    ...
    L.append(f"_Window: {start} to {today} ({window_days} days)_")
    ...
    L.append("*Blindspots*")
    L.append("• Sales/revenue still NOT tracked in Hyros. True ROAS is impossible until HubSpot sale events are wired.")
...
def main():
    ...
    if args.save:
        out = Path(__file__).parent /f"report_v2_{today}.txt"
